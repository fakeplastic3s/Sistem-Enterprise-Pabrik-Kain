...
<section class="section dashboard">
    ...
    <div class="row ">
        <div class="col-lg-7 col-sm-4 col-md-5 ms-2">
            <a href="<?= base_url('JadwalPerawatan/tambahjadwalperawatan'); ?>" class="btn btn-success mb-2"><i class="bi bi-clipboard-plus"></i></a>
            <button onclick="window.print()" class="btn btn-primary mb-2"><i class="ri-printer-line"></i></button>
            <a href="<?= base_url('JadwalPerawatan'); ?>" class="btn btn-secondary mb-2"><i class="bi bi-arrow-clockwise"></i></a>
        </div>
        <div class="col me-2 pb-3">

            <form class="d-flex" role="search" action="JadwalPerawatan" method="POST">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive-md">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php $no = 1 ?>
                        <?php foreach ($getJadwalPerawatan as $isi) : ?>
                            <tr>
                                <td><?= $no; ?></td>
                                ...
                                <td class="text-center">
                                    <?php
                                    $tanggal = date('d M Y', strtotime($isi['tanggal_perawatan']));
                                    echo $tanggal
                                    ?>
                                </td>
                                ...
                                <td class="text-center">
                                    <a href="<?= base_url('JadwalPerawatan/edit/' . $isi['id_perawatan']); ?>" class="btn btn-success btn-sm"><i class="ri-clipboard-line"></i></a>
                                    <form action="/JadwalPerawatan/hapus/<?= $isi['id_perawatan']; ?>" class="d-inline">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" onclick="javascript:return confirm('Apakah anda ingin menghapus data jadwal perawatan?')" class="btn btn-danger btn-sm"><i class="bi bi-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                            <?php $no++ ?>
